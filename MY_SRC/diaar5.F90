...
CONTAINS
   ...
   SUBROUTINE dia_ar5( kt, Kmm )
      ...
      IF( iom_use( 'botpres' ) .OR. iom_use( 'sshthster' ) .OR. iom_use( 'sshsteric' ) .OR. iom_use( 'botpres2D' ) ) THEN
         !
...
!!gm
         ...
         CALL iom_put( 'botpres2D', zbotpres    )
   ...
   SUBROUTINE dia_ar5_init
      ...
      IF( l_ar5 ) THEN
         ...
         IF( iom_use( 'botpres' ) .OR. iom_use( 'sshthster' ) .OR. iom_use( 'sshsteric' ) .OR. iom_use( 'botpres2D' ) ) THEN
            INQUIRE( FILE = 'sali_ref_clim_monthly', EXIST = llok )
            IF( llok ) THEN

...
! qco like:           
         ztmp(:,:) = SUM(tmask(:,:,:)*(pe3_in(:,:,:)-e3t_0(:,:,:)), DIM=3) 
         DO_3D( 1, 0, 1, 0, 1, jpk )
            pe3_out(ji,jj,jk) = 0.5_wp * (  umask(ji,jj,jk) * (1.0_wp - zlnwd) + zlnwd ) * r1_e1e2u(ji,jj)    &
               &                       * (   e1e2t(ji  ,jj) * ztmp(ji  ,jj) + e1e2t(ji+1,jj) * ztmp(ji+1,jj)) &  
               &                       * e3u_0(ji,jj,jk) * r1_hu_0(ji,jj)
         END_3D
...
! qco like:           
         ...
         DO_3D( 1, 0, 1, 0, 1, jpk )
            pe3_out(ji,jj,jk) = 0.5_wp * (  vmask(ji,jj,jk) * (1.0_wp - zlnwd) + zlnwd ) * r1_e1e2v(ji,jj)    &
               &                       * (   e1e2t(ji  ,jj) * ztmp(ji  ,jj) + e1e2t(ji,jj+1) * ztmp(ji,jj+1)) &  
               &                       * e3v_0(ji,jj,jk) * r1_hv_0(ji,jj)
         END_3D
...
!         END_2D
! qco like:           
         ztmp(:,:) = SUM(umask(:,:,:)*(pe3_in(:,:,:)-e3u_0(:,:,:)), DIM=3) 
         DO_3D( 0, 0, 0, 0, 1, jpk )
            pe3_out(ji,jj,jk) =  (  umask(ji,jj,jk)* umask(ji,jj+1,jk) * (1.0_wp - zlnwd) + zlnwd )   & 
               &                 * 0.5_wp * r1_e1e2f(ji,jj)                                           &
               &                 * (e1e2u(ji  ,jj) * ztmp(ji  ,jj) + e1e2u(ji,jj+1) * ztmp(ji,jj+1))  & 
               &                 * e3f_0(ji,jj,jk) * r1_hf_0(ji,jj)
         END_3D
...
! qco like:
         ztmp(:,:) = SUM(tmask(:,:,:)*(pe3_in(:,:,:)-e3t_0(:,:,:)), DIM=3)
         DO jk = 1, jpk
            pe3_out(:,:,jk) = e3w_0(:,:,jk) * (1._wp + ztmp(:,:)*r1_ht_0(:,:)*tmask(:,:,jk))
         END DO
         !
...
! qco like:
         ztmp(:,:) = SUM(umask(:,:,:)*(pe3_in(:,:,:)-e3u_0(:,:,:)), DIM=3)
         DO jk = 1, jpk
            pe3_out(:,:,jk) = e3uw_0(:,:,jk) * (1._wp + ztmp(:,:)*r1_hu_0(:,:)*umask(:,:,jk))
         END DO
         !
...
!         END DO
! qco like:
         ztmp(:,:) = SUM(vmask(:,:,:)*(pe3_in(:,:,:)-e3v_0(:,:,:)), DIM=3)
         DO jk = 1, jpk
            pe3_out(:,:,jk) = e3vw_0(:,:,jk) * (1._wp + ztmp(:,:)*r1_hv_0(:,:)*vmask(:,:,jk))
         END DO
      END SELECT
      !
